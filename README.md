...
- **记录与判断分离**：数据库只存观察事实（果冻状分泌物 / 少量出血 / 经期血量），
  "现在处于什么状态"由推算器对全部历史推导——血量上来时 Day 1 自动回溯到这段出血
  的第一天，旧记录永远不用改。排卵 = 果冻连续段最后一天；PMS 由报告或症状确认触发。
- **三模式**：daily/pms/period，每天"应该吃哪些"随模式变化（PMS 加 B6、经期鱼油加量）。
